...
import Models.AccountDAO.Factory.AccountDAO;
...
public class AccountDetailController extends HttpServlet {
    ...
    private final String AccountDetailsPage = "AccountDetails.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(!accountId.isEmpty()){
                account = accountService.getUserByAccountID(accountId);
                if(account != null){
                    request.setAttribute("AccountDetails", account);
                        
                }else{
                    url = accountController + "?action=Details";
                }
            }
        }catch (Exception ex) {
            log(ex.getMessage());
